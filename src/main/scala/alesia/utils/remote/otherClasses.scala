...
case class MsgStartExperiment(eID: ExpID) // EntryActor -> WorkerExperimentActor, signalizes that all Experiment Files have been sent, start of Experiment
...
case class MsgReady() // FileActor -> WEActor, when saving files is complete
